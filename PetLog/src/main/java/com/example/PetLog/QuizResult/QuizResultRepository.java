...
public interface QuizResultRepository extends JpaRepository<QuizResultEntity, Long> {
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM quiz_result WHERE user_id = :userId ORDER BY result_id DESC) WHERE ROWNUM = 1", nativeQuery = true)
    ...
    // 풀이시간 빠른 순 Top10
    @Query(value = """
    SELECT * FROM (SELECT qr.result_id, qr.result_score, qr.result_rank, qr.result_time, qr.user_id, qr.quiz_id, qr.get_grape, u.user_login_id FROM quiz_result qr
    ...
    // 중복 저장 방지
    @Query(value = "SELECT count(*) FROM quiz_result WHERE user_id = :userId AND quiz_id = :quizId", nativeQuery = true)
